...
2.0*n*n*n/t/1e9,t,cref[n/2][n/2-1]);
  ...
  dim3 dimGrid(n/TILE_WIDTH, n/(2*TILE_WIDTH));
...
__global__ void test_kernel(int N, double *A, double *B, double *C)
{
// using shared memory for storing TILES corresponding to this block in A & B. Each block is responsible for computing TILE [2i][j] & TILE [2i+1][j] in C
