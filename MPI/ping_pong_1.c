...
int main(int argc, char *argv[]) {
   ...
   for(i=1; i<num_procs; i++) { 
    if(rank == 0) {
        ...
        for(j=0; j<niter; j++) {
            MPI_Send(bufferA, msgsize, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
            MPI_Recv(bufferB, msgsize, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &status);
            MPI_Send(bufferB, msgsize, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
            MPI_Recv(bufferA, msgsize, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &status);
        } 
        ...
        //time = total_time/(4*niter);
        //bw = (8*msgsize)/time;
        // performance = (4*niter*8*msgsize)/total_time;
        ...
        for(j=0; j<niter; j++) {
        MPI_Recv(bufferA, msgsize, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
        MPI_Send(bufferA, msgsize, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        MPI_Recv(bufferB, msgsize, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
        MPI_Send(bufferB, msgsize, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD); 
        }
        MPI_Get_processor_name(proc_name, &len);
